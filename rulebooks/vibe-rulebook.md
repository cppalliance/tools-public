...
Mark each step complete in the plan file itself as its commit lands: flip the frontmatter todo's `status` to `completed` when the plan carries frontmatter todos; when it does not, append the commit hash to the step's own line or heading. A host-provided session todo tool mirrors the plan file but never substitutes for it - the tool's list dies with the session, while the plan file is what a fresh, compacted, or resumed session reads to learn where the run stands. Do not start the next step until every item is completed or cancelled. Main tracks the open-findings count from each Review return; before declaring the run done, report any open findings to the user.

Make no make-work commits; fold the fix back into the commit it corrects. An open finding of any severity blocks the next step: fix it, or reject it with a stated reason, or stop and re-plan. No step is declared done without naming the verification that ran - the test command and its result line; a completion claim without fresh evidence is a review failure. Stop only when no forward path exists. Apply rule 2 for hard-to-reverse choices. Commits are reversible - never stop for ordinary user confirmation.
...
**Review-and-Fix Subagent.** Run immediately after the coder finishes. Apply `<code-review>` and any plan-local review block. Before overwriting `vibe-review.md`, read the existing file and carry every still-open finding into the new file verbatim, marked with the step it came from; a finding leaves the file only when fixed or rejected with a stated reason. Write new failures as one-sentence entries. Apply fix rounds until no finding remains open - Critical first, then Important, then Minor - capped at three rounds. When a fix round changed anything, run `<code-review>` over the fix's own diff; what it finds enters the next fix round. If findings remain open after the third round, return blocked with what is left. If a fix round changes the tests, return an updated test command string. Return under 1,000 tokens: new findings, findings carried forward, findings closed, each count by severity, files changed, path to `vibe-review.md`.
