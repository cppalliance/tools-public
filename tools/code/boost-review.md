...
<img src="images/boost-review.png" alt="Boost Review" width="100%">
...
Intermediate files (`boost-review-{library-slug}.cache.md`, `boost-review-{library-slug}.tmp.md`) are **scratch**. The finished report `boost-review-{library-slug}.md` is **output**. If a report with this name already exists, increment the version suffix: `-v2`, `-v3`, etc.

Write to the `.tmp.md` scratch file first. Rename to the output filename only after the report is complete. On resumption, check for `.tmp.md` and continue from its tail.
