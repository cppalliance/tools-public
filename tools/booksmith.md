...
- **Large (over 50,000 words):** one Pourer per chapter, sequential; raw prior prose no longer fits, so the rolling state is load-bearing; each Pourer receives the rolling state plus only chapter N-1's prose for seam continuity.
...
- **Pourer.** Objective: write one chapter. Inputs: the chapter's bible entry; its pour map; the template sentences (full text, from the index); character registry entries for the chapter; the rolling state; chapter N-1's prose (when it exists); the brief's theme notes; the craft reference; the Pouring Discipline (verbatim). Apply the discipline to poured beats and the craft reference to all beats. Self-check per the leak test before returning. Write `chapters/ch-NN.md`.
