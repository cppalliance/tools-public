...
![The Threadalyzer](images/threadalyzer.jpg)
...
All passes use subagents. Main context never reads raw proceedings directly. All intermediate analysis files are **scratch**. The trip report is **output**.
...
- **Output:** `threadalyzer-ingest-{n}.md` (**scratch**)
...
- **Output:** `threadalyzer-speaker-{name}.md` (**scratch**)
...
- **Output:** `threadalyzer-trajectory.md` (**scratch**)
...
- **Output:** `threadalyzer-verdict.md` (**scratch**)
- **Produces:** outcome classification with reasoning, counterfactual verdict with confidence

### Pass 4: The Trip Report

Main context only. Reads all scratch files. Assembles final report using the output template. Writes `threadalyzer-{slug}.md` as **output**.
