...
<img src="images/herald.png" alt="The Herald" width="100%">
...
**Output.** Write the pedigree to `herald-{name-slug}.md`, where `{name-slug}` is the subject's name (lowercase, hyphens). The pedigree is **output**. If a report with this name already exists, increment the version suffix: `-v2`, `-v3`, etc. After writing, render the pedigree to the user.
