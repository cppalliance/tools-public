...
<img src="images/advocatus.png" alt="Advocatus Diaboli Tribunal" width="100%">
...
**How:** Delegate to a sub-agent (see Operational Directive). Search `reports/` for prior advocatus outputs (files prefixed with `advocatus-`), conversation history for prior red-team sessions on the same paper or domain, and MCP for any indexed prior causes. The sub-agent returns: prior testimony still in force, prior findings still relevant, questions already answered. The main agent imports what applies and discards what has been superseded by revision.
